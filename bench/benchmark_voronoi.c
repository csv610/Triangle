...
int main(int argc, char **argv) {
    ...
    */
    start = get_time();
    triangulate("zvQ", &in, &out, &vorout);
    end = get_time();

    printf("Voronoi Diagram Benchmark (In Circle)\n");
    printf("Points: %d\n", n);
    printf("Voronoi vertices: %d\n", vorout.numberofpoints);
    printf("Voronoi edges: %d\n", vorout.numberofedges);
    printf("Time: %f seconds\n", end - start);

    free(in.pointlist);
    if (out.pointlist) trifree(out.pointlist);
    if (out.trianglelist) trifree(out.trianglelist);
    if (vorout.pointlist) trifree(vorout.pointlist);
    if (vorout.edgelist) trifree(vorout.edgelist);
    if (vorout.normlist) trifree(vorout.normlist);
